...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Reduced_F = ObserveEvidence(F, E);
for i = 1:length(Reduced_F) 
Reduced_F(i).val = Reduced_F(i).val ./ sum( Reduced_F(i).val );
end
Joint = ComputeJointDistribution(Reduced_F);
M_out = setdiff(Joint.var,V);
M = FactorMarginalization(Joint, M_out);
