...
%First, fill factors / CPDs for genotypes

for i = 1:numPeople

if sum(pedigree.parents(i,:) == [0,0]) == 2

factorList(i) = genotypeGivenAlleleFreqsFactor(alleleFreqs, i); %A person with no parents
else 
factorList(i) = genotypeGivenParentsGenotypesFactor(numAlleles, i, pedigree.parents(i,1), pedigree.parents(i,2));
end
end

%Second, fill factors / CPDs for phenotype

for i = 1:numPeople

factorList(i+numPeople) = phenotypeGivenGenotypeFactor(alphaList, i, numPeople + i);
