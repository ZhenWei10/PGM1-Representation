...
Indx_val = IndexToAssignment(1:prod(phenotypeFactor.card),phenotypeFactor.card);

Disease_idx = Indx_val(:,1) == 1; 

Sub_val = Indx_val(Disease_idx,[2,3]);

for i = 1:size(Sub_val)(1)
Disease_prob(i) = alphaList(allelesToGenotypes(Sub_val(i,1),Sub_val(i,2)));
end

phenotypeFactor.val(Disease_idx) = Disease_prob;

phenotypeFactor.val(!Disease_idx) = 1-Disease_prob;
