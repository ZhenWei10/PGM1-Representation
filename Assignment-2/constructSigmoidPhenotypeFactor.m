...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
allele_num_G1 = length(alleleWeights{1});
allele_num_G2 = length(alleleWeights{2});

% Fill in phenotypeFactor.var.  This should be a 1-D row vector.
phenotypeFactor.var = [phenotypeVar,geneCopyVarOneList',geneCopyVarTwoList'];
% Fill in phenotypeFactor.card.  This should be a 1-D row vector.
phenotypeFactor.card = [2,allele_num_G1,allele_num_G2 ,allele_num_G1,allele_num_G2];

phenotypeFactor.val = zeros(1, prod(phenotypeFactor.card));
% Replace the zeros in phentoypeFactor.val with the correct values.
Design_M_all = IndexToAssignment(1:prod(phenotypeFactor.card),phenotypeFactor.card);
indx_succ = Design_M_all(:,1) == 1;
Design_M_suc = Design_M_all(indx_succ,:);

for i = 1:size(Design_M_suc)(1)
logit = alleleWeights{1}(Design_M_suc(i,2)) + alleleWeights{2}(Design_M_suc(i,3)) + alleleWeights{1}(Design_M_suc(i,4)) + alleleWeights{2}(Design_M_suc(i,5));
prob(i) = computeSigmoid(logit);
end
phenotypeFactor.val(indx_succ) = prob;
phenotypeFactor.val(!indx_succ) = 1-prob;
