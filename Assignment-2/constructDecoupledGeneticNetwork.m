...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
for i = 1:numPeople 
%Fill in phenotypes Given Copies factor
  factorList(i*3 - 2) =  phenotypeGivenCopiesFactor(alphaList, numAlleles, i, numPeople + i, 2*numPeople+1);
%Judge whether this person have parent infor
  if  all(pedigree.parents(i,:) == [0,0])  % No parents
 factorList(i*3 - 1) = childCopyGivenFreqsFactor(alleleFreqs, i);
  factorList(i*3) =  childCopyGivenFreqsFactor(alleleFreqs, numPeople + i);
  else  %With parents
  factorList(i*3 - 1) = childCopyGivenParentalsFactor(numAlleles, i, 
                                  pedigree.parents(i,1), numPeople + pedigree.parents(i,1)); %Mother
                                  
 factorList(i*3) =  childCopyGivenParentalsFactor(numAlleles, numPeople + i, 
                                  pedigree.parents(i,2), numPeople + pedigree.parents(i,2)); %Father
  end
end
