...
Assign_fac = IndexToAssignment(1:prod(factor.card), factor.card);

indx_equal = Assign_fac(:,1) == Assign_fac(:,2);

factor.val = ones(prod(factor.card),1);

factor.val(indx_equal) = ImageSimilarity(images(i).img, images(j).img);
